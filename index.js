...
async function connectToWA() {
  console.log("CONNECTING UNIQUE MD BOT🧬...");
  const { state, saveCreds } = await useMultiFileAuthState(__dirname + '/Unique_session/');
  ...
  const sendBotImage = (up) => {
      ...
      if (fs.existsSync(localImgPath)) {
          ...
          conn.sendMessage(conn.user.id, { image: imageBuffer, caption: up });
          console.log('Sent bot image from local file.');
      } else {
  ...
  conn.ev.on('connection.update', (update) => {
      ...
      if (connection === 'close') {
          ...
          console.log('♻️ Reading plugins, please wait...');
          const path = require('path');
          ...
          console.log('Bot connected, sending resources to express server...');

          let up = `*Connected to your chat*`;
          sendBotImage(up); // Call function to send the bot image
      }
  });
...
//============================================================================
app.get("/", (req, res) => {
  res.sendFile(path.join(__dirname, 'Unique_website/index.html'));
});
